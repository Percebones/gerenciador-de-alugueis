...
- Add an extra sanitizing step: test arguments passed to `certificateFor` with a (fairly permissive) regular expression
  limiting them to legal domain name chars
...
- [#48](https://github.com/davewasmer/devcert/pull/48): Enhance uninstallation and upgrade routines to revoke old certs
...
* [#45](https://github.com/davewasmer/devcert/pull/45): Set generated certificate to last 825 days, a limit imposed by
  OSX Catalina
...
* add -d flag to security command, not sure why it ignores -p otherwise, but oh
