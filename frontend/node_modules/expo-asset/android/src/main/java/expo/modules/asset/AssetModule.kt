...
class AssetModule : Module() {
    ...
    private suspend fun downloadAsset(appContext: AppContext, uri: URI, localUrl: File): Uri {
        ...
        if (appContext.filePermission?.getPathPermissions(appContext.reactContext, localUrl.parent)
                ?.contains(Permission.WRITE) != true
        ) {
        ...
        return withContext(appContext.backgroundCoroutineScope.coroutineContext) {
            try {
                val inputStream = when {
                    ...
                    uri.toString().startsWith(ANDROID_EMBEDDED_URL_BASE_RESOURCE) -> openAndroidResStream(
                        context,
                        uri.toString()
                    )
                ...
                inputStream.use { input ->
                    localUrl.outputStream().use { output ->
                        ...
                        if (bytesCopied == 0L) {
                            Log.w(
                                "ExpoAsset",
                                "Asset downloaded to $localUrl is empty. It might be conflicting with another asset, or corrupted."
                            )
                        }
                    }
                }
